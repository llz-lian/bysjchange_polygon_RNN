    ...
    def __getitem__(self, index):
        ...
        if point_num < self.length - 3:
            for points in polygon:
                index_a1 = int(points[0]/2)
                index_b1 = int(points[1]/2)
                index_a = int(points[0] / 8)
                index_b = int(points[1] / 8)
                index = index_b * 28 + index_a
                label_array[point_count, index] = 1
                real_index = index_b1*112+index_a1
                if(real_index>12544):
                    real_index = 12544
                label_index_array[point_count] = real_index
                point_count += 1
            label_array[point_count, 28 * 28] = 1
            label_index_array[point_count] = 112*112
            for kkk in range(point_count + 1, self.length):
                if kkk % (point_num + 3) == point_num + 2:
                    ...
                    index_a1 = polygon[kkk % (point_num + 3) - 2][0] / 2
                    index_b = int(polygon[kkk % (point_num + 3) - 2][1] / 8)
                    index_b1 = polygon[kkk % (point_num + 3) - 2][1] / 2
                    index = index_b * 28 + index_a
                    index1 = index_b1 * 112 + index_a1
                label_array[kkk, index] = 1
                if(index1>12544):
                    index1 = 12544
                label_index_array[kkk] = index1
        else:
