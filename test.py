...
def test(net, dataset, num=float('inf')):
    ...
    if not os.path.exists(dir_name):#now:3
            ...
            if obj['label'] in selected_classes:
                ...
                for label in labels_p:
                    if (label > 12544):
                        break
                    vertex = (
                        ((label % 112) * 2.0 + 4) / scale_w + min_col, 
                        ((int((label) / 112)) * 2.0 + 4) / scale_h + min_row
                            )
    ...
    # print('iou score:{}'.format(iou_score))s
...
"""iou score:{
'person': 0.6684083844358165, 
'car': 0.8707878440063752, 
'truck': 0.8185263859386064, 
'bicycle': 0.6633151794442117, 
'motorcycle': 0.40047249409382385, 
'rider': 0.7053649056410781, 
'bus': 0.8554727326279539, 
'train': 0.834071509099694}
iou score:{
'person': 0.666543109200089, 
'car': 0.8297414668340546, 
'truck': 0.9019242968915204, 
'bicycle': 0.6849684132292828, 
'motorcycle': 0.5568228105906313, 
'rider': 0.6999026853580916, 
'bus': 0.8243237150586114, 
'train': 0.8441437426568894}
"""